...
class ImageCacheExt extends ImageCache {
  ...
  final RemoveAwareMap<Object, _CachedImage> _cache = RemoveAwareMap<Object, _CachedImage>();
  ...
  final RemoveAwareMap<Object, _LiveImage> _liveImages = RemoveAwareMap<Object, _LiveImage>();

  ImageCacheExt() {
    _pendingImages.hasRemovedCallback = _hasImageRemovedCallback;
    _cache.hasRemovedCallback = _hasImageRemovedCallback;
    _liveImages.hasRemovedCallback = _hasImageRemovedCallback;
  }

  Set<Object> _waitingToBeCheckedKeys = Set<Object>();
  bool _isScheduledImageStatusCheck = false;

  void _hasImageRemovedCallback(dynamic key, dynamic value) {
    if (key is ImageProviderExt) {
      _waitingToBeCheckedKeys.add(key);
    }
    if (_isScheduledImageStatusCheck) return;
    _isScheduledImageStatusCheck = true;
    //We should do check in MicroTask to avoid if image is remove and add right away
    scheduleMicrotask(() {
      _waitingToBeCheckedKeys.forEach((key) {
      ...
      _waitingToBeCheckedKeys.clear();
      _isScheduledImageStatusCheck = false;
    });
  }
